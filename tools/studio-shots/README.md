...
The `app-*.webp` and `guide-*.webp` images come from a sandboxed CartoVox
studio, never from a live library. Copy a few saved worlds into a throwaway
data directory, start the app on a spare port, and run the passes in order:
...
python3 04-handbook.py        # the handbook's own set: shell, dock menus, canon sheets
python3 05-field-guide.py     # every app-* shot the landing page and field guide use
...
against 0.8.3 and reach for a toolbar 0.9 removed. It takes all twenty-nine
...
the map-only `chain-*` crops and for the helpers and prose 04 and 05 import.
...
`04-handbook.py` writes `guide-*.png`, which `build-images.py` keeps under
that name rather than prefixing `app-`. The two families document different
builds — the handbook is 0.9, the landing page and field guide are older — so
refreshing one must not silently restate the other. It opens the world named in
its `WORLD` constant, which has to exist in the sandbox library.
